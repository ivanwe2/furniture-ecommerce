...
|---|---|---|---|
| 2026-09-02 | **Hero wordmark now aligns flush with the headline — the `text-indent` is gone.** Client feedback via Ivan: the wordmark and the title „are not on the same line on the left", which he suspected was a font problem. It was not: `Wordmark` carried `textIndent: tracking`, which at the hero's 46px and 0.2em pushed the first glyph **9.2px** to the right of the container edge the headline starts from. That indent was correct once — CSS letter-spacing adds its space AFTER every character including the last, so a CENTRED mark sits one tracking-unit left of true centre, and the indent cancelled it for the sign band. With the band deleted every remaining caller (hero, header, footer) is **left-aligned**, where the same rule is a defect rather than a correction. Removed outright rather than gated behind a prop — no centred caller exists to serve, and it can come back with one if a centred use returns. Header and footer marks shift left by their own tracking too, which is likewise a fix: both now sit flush with the nav and the tagline beneath them. Verified by measuring **ink** edges, not boxes, via canvas `actualBoundingBoxLeft` at 1280 and 375 — boxes flush at 0px, ink within 1px of the headline (side bearing between two different typefaces, i.e. optical parity). | client feedback | src/components/layout/Wordmark.tsx · here |
| 2026-09-01 | **Only the navbar's WORDMARK hides on the landing page — never the bar.** Ivan noticed the hero mark and the navbar mark sit on screen together on `/` and proposed bringing back the reveal-on-scroll bar to avoid duplicating the logo. Pushed back on hiding the whole bar and he agreed: with the sign band gone the hero now starts at the top of the page, so hiding the bar would open the landing page with **no menu and no cart at all** — worse on a phone than the duplication it fixes — and it also runs against what the client had just asked for („по-скоро да си остане както преди с бара"). Hiding only the mark solves the stated problem exactly (one mark on screen at a time) at a fraction of the risk. **State is derived, not synced:** `isLanding = pathname === '/'` combined with a `scrolledPast` flag, so a route change needs no effect to fix state up and there is no frame on load where both marks show before an effect hides one — the failure mode a `useState` seeded from the pathname would have had. The sentinel sits at the base of the hero mark; its absence fails open, and a page too short to scroll past it shows the mark anyway. The hidden link **keeps its space** (so the nav does not shift when it fades in) and is taken out of the tab order and the a11y tree rather than left as an invisible target — defensible because the only moment it is hidden is at the top of the landing page, where a „go home" link is redundant. **Deliberately no `:focus-visible` pin this time:** nothing focusable is being hidden that a keyboard user needs, which is what made the previous version's click-pin bug possible at all. Verified 6 states x 2 widths, sampling after the 300ms fade settles: `/` at top (mark 0, bar visible, cart present), scrolled (1), **back to top (0 again — the old click-pin regression does not recur)**, `/brands` (1), `/cart` (1, the short-page case), and **client-side nav `/`→`/contact` (1, the detached-sentinel case that bit last round)**. Zero console errors. | Ivan's proposal, amended in discussion | src/components/layout/Header.tsx · (site)/page.tsx · here |
| 2026-09-01 | **Company wordmark placed at the head of the hero, landing page only.** The client annotated a screenshot with a large „НАСТЕХ" drawn in above the „МЕБЕЛЕН ОБКОВ · ПРОИЗВОДСТВО И ТЪРГОВИЯ" eyebrow — that is where the mark goes now that the full-width band is gone („надписа на фирмата да отиде тук"). Rendered in `text-ink` on the cream hero rather than the client's white annotation ink, which was drawing over a light background and would be invisible. Only on `/`; the navbar carries the mark on every other route. **Sizes are computed from the container, not chosen by eye:** Factor Expanded is a very wide face and the 0.2em tracking pushes the rendered mark to roughly 10x the font size, so the first attempt at `10.5vw` ran the final X off the right edge on a phone. Measured the inner glyph span (the outer element is `block`, so its box is the container width and tells you nothing) at 320/375/640/1280: 241/282/386/468px against 288/343/592/576px of usable width — fits everywhere with the X intact. | client feedback (annotated screenshot) | src/app/(site)/page.tsx · here |
| 2026-09-01 | **Hero animation restored to the R3 butt hinge, then polished.** Round-6 client feedback, relayed by Ivan: they want „the previous animation, but a bit more polished". Two animations have been shown to them, so which one „previous" meant was ambiguous, and the older one **contradicts their own earlier instruction** — they praised it („сегашната анимация е супер") but told us „просто не предлагаме такъв тип", which is exactly why it was replaced by the SEVROLL concealed hinge. Raised with Ivan rather than guessed; **Ivan chose the R3 butt hinge**, so the hero art is now a part the shop does not stock (a decorative backdrop at 42% opacity, not a listing — their call to make). Restore itself was cheap: identical `startHinge` signature, identical `HingeDeps`, same three@0.160.1, so `git checkout` of the scene plus the two stage-opacity values (0.52/0.58 back to 0.42/0.5, which had been raised only for the black SEVROLL body). **The polish is fidelity, not redesign** — geometry, materials and cycle timing are the ones they approved: (1) **soft shadows** (`PCFSoftShadowMap`, 2048 map, radius 3, shadow camera tightened from the ±5 default onto what the subject actually occupies) — the scene previously had shadows switched off entirely, and parts shading each other is what stops an assembly of primitives reading as one; (2) a **micro-surface bump map** over the existing brushed roughness, so highlights break up instead of sitting on mathematically perfect planes; (3) **crevice rings** in the 10% gap the knuckle segments already left between them, which had been seeing straight through to the background and making the barrel read as one smooth tube; (4) a tighter plate chamfer. **Two real defects fixed, not just looks.** The camera sat at a fixed z with a fixed VERTICAL fov, so the framing was only ever right at the desktop stage's 0.88 aspect — measured in-page as 0.88 desktop vs 1.027 mobile — and the exploded parts (leaves 1.7 out in x, pin 2.0 up in y) left the frame on a phone. Horizontal fov is now held constant across aspects and the distance is solved per state by projecting the box corners and correcting, clamped so the fit may only pull BACK: the assembled hinge keeps the size and position the client signed off on, and only the explode opens up. Second: the leaves are FLAT and were `metalness: 1`, which has no diffuse term at all, so whichever leaf caught a bright quadrant of the environment washed out to paper white and vanished against the cream page, worst of all mid-explode — 0.88 gives them a floor to sit on while the reflection still does most of the work. **Verified against a production build at 1280 and 375** (see the entry above for why not `pnpm dev`): scene starts, no console errors at either width, no horizontal overflow, exploded parts in frame on mobile. | client feedback; Ivan's call on which animation | src/components/home/hinge-scene.ts · globals.css · here |
| 2026-09-01 | **Middleware no longer intercepts `/_next/webpack-hmr`.** The matcher excluded `_next/static` and `_next/image` but nothing else under `_next`, so in dev it matched the Fast Refresh websocket and broke its handshake (`ERR_INVALID_HTTP_RESPONSE`, repeating). Now `_next` is excluded wholesale, with the same `(?:/|$)` boundary the `api` alternative uses so `/_nextdoor` stays gated; the test pins webpack-hmr, `/_next` bare, and that look-alike. Nothing under `_next` ever needed the site lock — App Router sends RSC requests to the page's own URL with `?_rsc=`, not to a `_next` path, so those remain gated. **Honest scope:** this was found while chasing a dev server on which NOTHING hydrated, and it did **not** fix that — hydration still failed after the change and a restart, and the same build hydrates correctly under `next start`. So the hydration failure is a separate `pnpm dev`/Turbopack problem, still unexplained, and visual verification was done against a production build instead. The matcher fix stands on its own merits regardless. | found while verifying | src/middleware.ts · src/middleware.test.ts · here |
| 2026-09-01 | **Sign band removed; the navbar is back to always-visible.** Round-6 client feedback: „честно казано не съм много фен на надписа, много натрапчиво ми седи — по-скоро да си остане както преди с бара". The full-width fascia between the info strip and the navbar is deleted, and with it the whole reveal-on-scroll mechanism it existed to drive — the `IntersectionObserver`/`ResizeObserver` sentinel, the `pathname`-keyed effect, the `:focus-visible` pin and the translate/opacity transition. **All of that machinery was only ever needed because the bar had to hide behind the band**; with no band there is nothing to hide behind, so removing it is a simplification rather than a loss, and it retires three previously-fixed bug classes along with it (detached sentinel after a client-side nav, short unscrollable pages left with no navigation at all, the click-focus pin). **The navbar wordmark stays** — the client approved it explicitly last round („thank you for the weight it is nice exactly what i meant"), so only the band was ever at issue. The company mark is to be relocated to a spot the client is sending a screenshot of; that placement is deliberately NOT guessed here. | client feedback | src/components/layout/Header.tsx · SignBand.tsx (deleted) · (site)/layout.tsx · here |
| 2026-08-28 | **BUG: returning home via the header logo left the navbar pinned open on top of the band.** Ivan: „when I move around the pages and go back to the home page using the logo, the navbar and banner both appear — the navbar doesn't disappear when I scroll to the top like normally". Reproduced and then **isolated by contrast**, which is what named the cause: reaching `/` through the logo (a link INSIDE the bar) left `header` opacity 1 at scrollY 0 with `document.activeElement` still on that link, while reaching the same page through the breadcrumb (a link outside the bar) correctly gave opacity 0. The culprit was my own accessibility safeguard: `onFocusCapture` set a `tabbedInto` flag so a keyboard user tabbing into the hidden bar brings it back — but **a mouse click focuses too, and nothing ever cleared the flag**, so one click on the logo pinned the bar open for the rest of the session. Fixed on both sides rather than by deleting the safeguard: the flag is now set only when the focused element matches **`:focus-visible`** (keyboard focus, not a click), and `onBlurCapture` clears it when focus leaves the bar. Regression-checked all four states — mouse-via-logo then top of `/` (hidden), then scrolled (visible), keyboard at top before tabbing (hidden), keyboard tabbed in (revealed) — because fixing the click path by removing the focus reveal would have quietly cost keyboard users the navigation. **Testing note:** the keyboard assertion first reported a failure at opacity `0.908`; that was the 300ms transition still running, not a defect — sample after it settles. | client-reported bug | src/components/layout/Header.tsx · here |
| 2026-08-26 | **Top-level category in the header is now a link, not a button.** Ivan: clicking „Мебелен обков" only opened the dropdown and never went anywhere, „makes it seem like the button isn't working and the app is dysfunctional". He was right, and it is a defect rather than a preference: the trigger was a `<button>` whose only job was toggling the panel, and because the panel lists just the CHILDREN there was **no route to the parent category page from the header at all** — while that page is the useful destination, rendering the „Подкатегории" grid AND every product beneath the category with sorting and brand filters (27 category links + 20 products on `mebelen-obkov`). The **mobile drawer already did this correctly** (name links, chevron expands), so the desktop nav was also inconsistent with the app's own pattern. Now: the item is a `<Link>` that navigates; hover/focus still opens the panel; and the panel gained a „Виж всички · <name> →" row so anyone who opens it instead of clicking through can still reach the parent. Panels only open for categories that actually have children. Closing happens in the links' `onClick` rather than an effect keyed on the pathname — setting state straight from an effect body cascades renders. **Dropped the `menubar`/`menuitem` roles:** that ARIA pattern is for application menus and hijacks the arrow keys; site navigation is a list of links with a disclosure (`aria-expanded` + `aria-controls`), which is simpler and better behaved. Escape closes; outside-click closes. Verified: trigger resolves as a link, hover opens (12 links in the panel), click lands on `/category/mebelen-obkov` with the subcategory grid and 20 products, panel closes on navigation, no console errors. **Note on a false alarm:** the first automated check reported the destination had no „Подкатегории" section — it does; the assertion compared against `innerText`, which reflects the CSS `uppercase`, so it failed on case rather than on content. | client feedback | src/components/layout/MegaMenu.tsx · here |
| 2026-08-26 | **Hinge back to the product's black, and de-gamified.** Follow-up to the entry below. Ivan: „направи го малко по-черно" and „изглежда малко като от игра". **The earlier conclusion that black could not work here was contaminated by the two rendering bugs recorded below** — with the inverted normals and the `metalness: 1` on flat faces fixed, a dark body shades properly, so the finish went back to the SKU's black (`0x24201b`). It is held legible by METAL rather than by lightening the body: steel cross-head screws, a **chromed** cover cap as on the real part (a large flat brass panel there read as toy gold), and a single brass pivot pin kept as the site accent. Stage opacity 0.42→0.52 (0.5→0.58 on mobile) — a dark subject at the old value washes out to grey against cream. **„Gamified" was treated as three concrete causes, not as taste:** (1) *uniform materials* — added a procedural micro-surface used as BOTH bump and roughness map (speckle, directional tooling marks, occasional deeper scratches), so highlights break up instead of sitting on mathematically perfect planes; (2) *over-soft edges* — bevel 0.05→0.028 with more segments, because stamped steel carries a small chamfer, not a rolled edge; (3) *flat lighting* — 2048 shadow map with a softening radius, stronger key and rim, and a dim bounce from below, without which a dark body loses its underside entirely and reads as a flat cut-out. | client feedback | src/components/home/hinge-scene.ts · globals.css · here |
| 2026-08-26 | **Hinge scene revisited: boards removed, exploded view restored, and TWO rendering bugs found that explain the „completely black" complaint.** Round-5 feedback: „I can see some sort of white backing", „the item is not properly oriented", „it being completely black messes with the design — maybe match the banner colours", „where did the disassembly from the previous animation go?", and „realistic means as if it's a real thing". **The white backing** was the cabinet door and carcass: they never read as a cabinet, only as an unidentifiable slab behind the mark. Deleted. Without them the linkage articulates in mid-air, which still shows the mechanism, and the scene returns to the floating-technical-illustration language the rest of the hero speaks. **The disassembly** was in the butt-hinge scene this replaced and was dropped when it was rewritten — restored as a full explode/reassemble beat with per-part offset vectors, plus a slow turntable. **„Completely black" turned out to be two real bugs, not a styling choice.** (1) `extrudeSide` built its shapes **clockwise**, so `ExtrudeGeometry` derived inverted normals and every extruded part — arm, links, plate, cap — rendered unlit regardless of its material colour. Winding is now normalised by signed area rather than trusting each call site. (2) The bigger one: those parts were `metalness: 1`, and a **flat** face at full metalness has no diffuse term — it shows only what it reflects, and a flat plate reflecting a dark quadrant of `RoomEnvironment` renders black however light its colour. Only the *curved* cup escaped it, which is exactly what the screenshots showed. Backing metalness off to ~0.5 gave the large flat parts an albedo again. Both were invisible to „set a lighter colour" — the colour was already light. **Finish** is now mid-tone machined metal with a charcoal cover cap (echoing the fascia) and brass pins, rivets and cap — the accent the site already uses. Deliberately not the catalogue black: on cream it collapses to a silhouette; light metal goes faint at the stage's 0.42 opacity; mid-tone holds both. **Orientation/centring:** the camera now views ACROSS the separation axis (parts spread along z, so looking down that axis stacked them), and there are **three** measured framings — closed, open, exploded — interpolated in cycle order, because a single box spanning the whole cycle is the union of its widest moment and leaves every other moment small inside it. | client feedback | src/components/home/hinge-scene.ts · here |
| 2026-08-26 | **Sign band is landing-only, and the reveal now works the same on phones as on desktop.** Round-5 feedback: „why isn't mobile like desktop — banner at top, then the navbar after scrolling?" and „the large banner doesn't need to be on every page, that's what the navbar is for — only on landing". Both applied, and together they make the model self-consistent: **landing** = band at the top with the bar hidden behind it at *every* width, sliding in once the band scrolls away; **every other page** = no band, so the bar is present the moment the page loads. The previous `lg:` gate is gone — it existed only to stop mobile losing its burger and cart above the fold, and that concern disappears once inner pages always show the bar and the band is the deliberate top of the landing page. `SignBand` became a client component to read `usePathname`; the band's *absence* is now load-bearing, which the header's existing fail-open already handled. **One real bug this shook out:** the header set up its `IntersectionObserver` once on mount, so a client-side navigation from `/` to an inner page left it observing a **detached** sentinel and the bar would have stayed hidden forever on a page with no band. The effect is now keyed on `pathname`. Caught by adding a client-side-navigation case to the sweep, not by review. Verified across 4 widths × 4 routes + a nav case at each width: band only on `/`, bar hidden at the top of `/` at every width and revealed after scroll, bar visible immediately on all inner pages, pinned wherever the page can scroll far enough, no horizontal overflow, no console errors. | client feedback | src/components/layout/SignBand.tsx · Header.tsx · here |
| 2026-08-25 | **Order/contact emails restyled to the new fascia, and given their first tests.** Ivan: „make sure the emails style is up to date and perhaps uses the new logo". The header was a bronze left-aligned lockup; it is now the shopfront — the mark letter-spaced and light on the dark bar, mirroring `--color-dark`/`--color-on-dark`. **The mark stays Cyrillic „НАСТЕХ" and deliberately does NOT use the site's „HACTEX":** the storefront can render Latin look-alikes because it loads Factor, but **email clients strip webfonts**, so the look-alikes would fall back to a normal face and arrive as a meaningless Latin word. The background sits on the `<td>` rather than a `<div>` because Outlook drops backgrounds off block elements, and `color-scheme`/`supported-color-schemes` are declared so dark-mode clients stop re-colouring the bar. **First test coverage for the email path** (`src/emails/send.test.ts`, nodemailer + `server-only` stubbed, module re-imported per case so the SMTP config is read from stubbed env): asserts the fascia markup, that „HACTEX" never appears, that both colour schemes are declared, and — the one that matters — that customer-supplied name/email/message are **escaped** before landing in the owner's inbox, including that a quote in the address cannot break out of the `mailto` attribute. That control was described in SECURITY.md but had nothing pinning it. | client feedback | src/emails/send.ts · src/emails/send.test.ts · here |
